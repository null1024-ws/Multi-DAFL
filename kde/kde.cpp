...
double KDE::get_01_distr_prob(){
    
    int total_true = sum_x_map[0];
    int total_num = count_map[0];
    return (double)total_true / total_num;
}

double KDE::estimate_01_likelihood(){
	// alpha/beta represents the bias to 1/0
	double alpha = 1, beta = 5;

	// The data_matrix[0] contains all of the data, 
	// alpha represent the possibility to enter the condition, beta represent the possibility to pass the condition
	vector<double> data = data_matrix[0];
	for (double z: data){
		if (z == 0){
			beta += 1;
		} else {
	...
	// // 计算 Beta 分布归一化常数
	// printf("[KDE]-estimate-01-likelihood: beta is: %lf, alpha is: %lf\n", beta, alpha);
	// double p = 0.5;
    // double log_beta_function = std::lgamma(alpha) * std::lgamma(beta) / std::lgamma(alpha + beta);

	// printf("[KDE]-estimate-01-likelihood: beta_function: %lf\n", log_beta_function);

	// double pdf_log = (alpha - 1) * std::log(p) + (beta - 1) * std::log(1 - p) - log_beta_function;
	// printf("[KDE]-estimate-01-likelihood: pdf_log: %lf\n", pdf_log);

	// double pdf = std::exp(pdf_log);
	// printf("[KDE]-estimate-01-likelihood: pdf: %lf\n", pdf);
	// return pdf;

    // // 计算 Beta PDF
	// printf("[KDE]-estimate-01-likelihood: result is %lf\n", std::pow(p, alpha - 1) * std::pow(1 - p, beta - 1) / beta_function);
    // return std::pow(p, alpha - 1) * std::pow(1 - p, beta - 1) / beta_function;
	return alpha / (alpha + beta); // This is the everage value, which represent the possibility to pass the condition
}
