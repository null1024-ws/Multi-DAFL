...
// no_filename_match seems like a flag for instrumenting all functions
bool no_filename_match = false;
// vector contains map<file_name, set<function_name>> for all target files
...
void initialize(void) {
  ...
  if(select_files.size() != dfg_files.size()) {
    FATAL("The number of files in select folder and dfg folder are not equal");
  }
...
bool AFLCoverage::runOnModule(Module &M) {
  ...
  initialize();
  /*
  Today, I finished the initialize function.
  Tomorrow, finish the remaining part:
  1. The shared memory.
  2. The selective coverage condition
  3. the DFG scoring condition
  4. isdfg_node condition
  5. Remember to set the target number of target files as global variable
  -----
  Update in 5.13 
  1. Copy the existing code in my finished repo
  2. Revise the code in this repo to satisfy the design mentioned above.
  ...
  */
  ...
  for (auto &F : M) {
    ...
    */
    ...
    /* 
      the following code block is the old version of DAFL
    */
    /* Check if this function is our instrumentation target. */
    /* 
      selective_coverage means not all basic blocks are instrumented.
      When selective_coverage is true, finding files to be instrumented is first thing to do
      Then dfg_scoring decides whether there is dfg_scoring to instrument in this file.    
    */
    // if (selective_coverage) {
    ...
    for (auto &BB : F) {
      ...
      // We modify the scoring code here
      ...
      // What is the usage of the break?
